...
def scan_dir(dir_path):
    """
    Scan the directory and return the list of files.
...
def select_file(files):
    """
    Allow the user to select a file from the list of files.

    Args:
        files (list): The list of files to choose from.
...
def flash_files(files):
    """
    Flash the files using the fastboot command.

    Args:
        files (list): The list of files to flash.
    """
    for file in files:
        print(f"Flashing {file}...")
        run_command(["fastboot", "flash", "--slot=all", file, file])

def scan_and_select_files(dir_path):
    """
    Scan the directory and select files.
    ...
    selected_files = [select_file(files) for _ in range(3)]  # select up to 3 files
...
def main():
    ...
    while True:
        try:
            ...
            if selected_version < 1 or selected_version > len(firmware_versions):
                raise ValueError
    ...
    firmware_files = scan_and_select_files(os.path.join(firmware_dir, selected_folder))
    recovery_files = scan_and_select_files(recovery_dir)
    rom_files = scan_and_select_files(roms_dir)
    gapps_files = scan_and_select_files(gapps_dir)
    addon_gapps_files = scan_and_select_files(addon_gapps_dir)
    jailbreak_files = scan_and_select_files(jailbreak_dir)

    # Flash files
    print("Flashing files...")
    flash_files(firmware_files)
    flash_files(recovery_files)
    flash_files(rom_files)
    flash_files(gapps_files)
    flash_files(addon_gapps_files)
    flash_files(jailbreak_files)
